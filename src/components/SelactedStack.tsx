...
const SelactedStack = ({ selectedStacks ,setselectedStacks }: SelactedStackProps) => {
  ...
  return (
    <div className="p-6 border border-[#eee] rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] space-y-2">
      ...
      <button style={{background: 'var(--brand-gradent)'}} onClick={handleRemoveAllStack} className="cursor-pointer text-[12px] font-medium text-white border border-[#ED8C85] w-full py-2 rounded-md">Remove All</button>
    </div>
  );
};
